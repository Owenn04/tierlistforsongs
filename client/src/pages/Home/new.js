...
import DraggableBox from '../../components/Other/DraggleBox.js'; //lol dragglebox
import DropZone from '../../components/Other/DropZone.js';
...
const Home = () => {
    ...
    const handleDrop = (item, rowIndex, boxIndex) => {
        setBoxItems((prevItems) => {
            const newItems = [...prevItems];
            const draggedItem = newItems[item.rowIndex][item.index];
                        ...
                        {boxItems.map((row, rowIndex) => (
                            <div key={rowIndex} className="row">
                                {row.map((item, boxIndex) => (
                                    <DropZone key={boxIndex} rowIndex={rowIndex} boxIndex={boxIndex} onDrop={handleDrop}>
                                        <DraggableBox item={item} index={boxIndex} />
                                    </DropZone>
                                ))}
                            </div>
                        ))}
